...
function robustFindProduct(itemId) {
...
async function loadInventory() {
    try {
        ...
        if (!response.ok) throw new Error('No se pudo cargar inventario: HTTP ' + response.status);

        const data = await response.json();

        // Aceptar ambos formatos: { productos: [...] } o directamente [...]
        inventario = (data && data.productos) ? data.productos : data;

        // Exponer global para otros módulos
        try { window.inventarioGlobal = inventario; } catch (e) {}

        console.log('[Checkout] Inventario cargado:', Array.isArray(inventario) ? inventario.length : inventario);
    } catch (error) {
        console.error('[Checkout] Error inventario:', error);
        inventario = []; // asegurar que inventario sea array
    }
}
...
function renderCheckoutCart() {
    ...
    container.innerHTML = cart.map(item => {
        ...
        let product = robustFindProduct(item.id);
